...
MODEL_MODULE_NAME = "bookmaker_kornerv12_multileague_league_config_refactor_v2"

# -------------------------------------------------
# Presets (Option 2: UI overrides + saved defaults)
# -------------------------------------------------
...
DEFAULT_PRESETS = {
    ...
    "LL": {
        ...
        "mu_league_anchor_low": 0.90,
        "mu_league_anchor_high": 1.10,
        "chaos_max": 0.02,
        "alpha_squeeze": 0.75,
        "margin": 0.08,
        "over_price_boost": 0.01,
...
def _effective_preset(league: str, saved: dict) -> dict:
    base = DEFAULT_PRESETS.get(league, {})
    ov = saved.get(league, {})
    out = dict(base)
    out.update(ov)
    return out
...
with st.sidebar:
    ...
    preset = _effective_preset(league, saved_presets)


    st.subheader("Run parameters")
    margin = st.slider("Margin", 0.00, 0.12, float(preset.get("margin", 0.08)), 0.005)
    over_boost = st.slider("Over price boost", 0.00, 0.10, float(preset.get("over_price_boost", 0.01)), 0.005)
    alpha_squeeze = st.slider("Alpha squeeze", 0.50, 1.20, float(preset.get("alpha_squeeze", 0.70)), 0.01)
    ...
    st.subheader("League tuning (saved per league)")
    emp_bias_k = st.slider("EMP_BIAS_K", 5.0, 120.0, float(preset.get("emp_bias_k", 35.0)), 1.0)
    emp_bias_max_factor = st.slider("EMP_BIAS_MAX_FACTOR", 1.05, 1.60, float(preset.get("emp_bias_max_factor", 1.25)), 0.01)
    soft_anchor_w_base = st.slider("SOFT_ANCHOR_W_BASE", 0.00, 0.30, float(preset.get("soft_anchor_w_base", 0.07)), 0.005)
    soft_anchor_w_upward = st.slider("SOFT_ANCHOR_W_UPWARD", 0.00, 0.40, float(preset.get("soft_anchor_w_upward", 0.14)), 0.005)
    mu_league_anchor_low = st.slider("MU_LEAGUE_ANCHOR_LOW", 0.80, 0.98, float(preset.get("mu_league_anchor_low", 0.88)), 0.005)
    mu_league_anchor_high = st.slider("MU_LEAGUE_ANCHOR_HIGH", 1.02, 1.25, float(preset.get("mu_league_anchor_high", 1.12)), 0.005)
    chaos_max = st.slider("CHAOS_MAX", 0.00, 0.20, float(preset.get("chaos_max", 0.04)), 0.005)
    ...
    with colP:
        if st.button("💾 Save preset"):
            saved_presets.setdefault(league, {})
            saved_presets[league].update({
                **overrides,
                "margin": float(margin),
                "over_price_boost": float(over_boost),
                "alpha_squeeze": float(alpha_squeeze),
            })
            _save_presets(saved_presets)
            st.success(f"Saved → {os.path.basename(PRESETS_PATH)}")
            st.rerun()
    with colQ:
        if st.button("↩ Reset preset"):
            if league in saved_presets:
                del saved_presets[league]
                _save_presets(saved_presets)
            st.warning("Reset to built-in defaults.")
            st.rerun()
